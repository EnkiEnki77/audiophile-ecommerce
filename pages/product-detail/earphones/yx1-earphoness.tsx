...
const YX1WirelessEarphones = () => {
  const itemData = data.filter(item => item.slug === 'yx1-earphoness')
  ...
  const onAddToCart = ( e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    setCart([{img:itemData[0].image, name:itemData[0].name.replace(' Wireless Earphones', ''), price: itemData[0].price, counter: counter}])
  }
